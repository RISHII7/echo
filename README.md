...
- **Real-Time Backend** — Convex reactive database with live queries and server mutations, bridged to Clerk sessions; `contactSessions` table with 24-hour TTL and browser metadata tracking for widget visitors
...
- **Voice AI** — Vapi AI SDK integration in the widget app with real-time call state, speech events, and live transcript; widget UI module with `WidgetView`, `WidgetHeader` (blue gradient), `WidgetFooter` (Home/Inbox nav bar), `WidgetAuthScreen` (name/email contact form), `WidgetLoadingScreen` (multi-step org + session validation), `WidgetSelectionScreen` (start chat entry point), and `WidgetChatScreen` (conversation view with back navigation); Jotai atom-based screen router with localStorage-persisted contact sessions; `conversations` Convex table with session-gated `create` mutation and ownership-verified `getOne` query
- **AI Support Agent** — `@convex-dev/agent`-powered chat agent (`google.chat("gemini-2.5-flash")` via `@ai-sdk/google`) with per-conversation threads, paginated message history, and a full AI Elements chat UI (conversation, message, input, response, reasoning, suggestion, tool, source, branch, dropzone); infinite scroll for chat history via `IntersectionObserver`, and Dicebear-generated assistant avatars
- **Widget Inbox** — paginated conversation list (`WidgetInboxScreen`) with relative timestamps, last-message previews, and color-coded `ConversationStatusIcon` (resolved/unresolved/escalated); Home/Inbox footer navigation wired to the widget screen router
- **Dashboard Inbox** — resizable two-pane `ConversationsLayout` with a status-filterable, infinite-scrolling `ConversationsPanel` (organization-scoped, Clerk-identity-gated), country-flag badges resolved from contact timezone, and active-route highlighting
- **Dashboard Chat** — operator-facing `ConversationIdView` for replying to widget conversations via the AI Elements chat UI, backed by identity-gated `private/conversations.getOne` and `private/messages` (create/getMany) Convex functions; three-state `ConversationStatusButton` (unresolved/escalated/resolved) and AI-powered "Enhance" reply drafting via `enhanceResponse`
- **AI Agent Tools** — the support agent can call `escalateConversationTool`, `resolveConversationTool`, and `searchTool` (RAG-backed knowledge base search with a dedicated result-interpretation pass) to transition a conversation's status or answer questions directly from the chat, with agent involvement automatically paused once a conversation is escalated or resolved; all agent prompts centralized in `system/ai/constants`
...
- **Vapi Plugin** — dashboard connection flow for Vapi voice AI credentials, encrypted at rest in AWS Secrets Manager (per-organization, per-service secret naming) via a `plugins` table tracking connection state
...
Create `apps/web/.env.sentry-build-plugin` (local-only, gitignored):

| Variable            | Description                                           | Required |
| ------------------- | ----------------------------------------------------- | -------- |
| `SENTRY_AUTH_TOKEN` | Sentry auth token used to upload source maps on build | No       |
...
Get your Sentry auth token from the [Sentry Dashboard](https://sentry.io) → **Settings → Auth Tokens**.
