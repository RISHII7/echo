...
- **AI Support Agent** — `@convex-dev/agent`-powered chat agent (`google.chat("gemini-2.5-flash")` via `@ai-sdk/google`) with per-conversation threads, paginated message history, and a full AI Elements chat UI (conversation, message, input, response, reasoning, suggestion, tool, source, branch, dropzone)
