...
- **Voice AI** — Vapi AI SDK integration in the widget app with real-time call state, speech events, and live transcript; widget UI module with `WidgetView`, `WidgetHeader` (blue gradient), `WidgetFooter` (Home/Inbox nav bar), `WidgetAuthScreen` (name/email contact form with browser metadata collection), and `WidgetLoadingScreen` (multi-step org + session validation flow); Jotai atom-based screen router with 8 named widget screens and localStorage-persisted contact sessions via `jotai-family`
